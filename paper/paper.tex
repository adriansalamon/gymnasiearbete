...
\begin{abstract}
	\noindent Various voting methods are in use across the world. They range from simple processes that everyone can understand to complex mathematical systems. This essay examines differences and similarities of the behavior of three voting methods: the single transferable vote (STV), first-past-the-post (FPTP), and Schulze method. This is done by implementing each voting method in Typescript, a programming language, and then running election data into them. This paper provides descriptions, pseudocode and implementations of each method. A program to generate voting results was developed in order to test the voting methods. The methods are evaluated in both singe-seat and multi-seat elections. No new findings are presented, but the behavior of each method is evaluated experimentally. Several conclusions are drawn about the behavior of the methods, including to what degree the different methods provided proportionate results.
\end{abstract}
\renewcommand{\abstractname}{Sammanfattning}
\pagebreak
\begin{abstract}
  \begin{center}
  	\noindent \textbf{Mjukvaruimplementationer och analys av tre valsystem}  
  \end{center} 
  \noindent Flera valsystem används i världen. De kan vara allt ifrån enkla processer som alla kan förstå till komplexa matematiska system. Denna uppsats undersöker skillnader och likheter mellan tre valsystem: Enkel Överförbar Röst, Majoritetsval och Schulze-metoden. Detta genom att implementera alla valsystem i programmeringsspråket Typescript och testa de med olika valdata. Denna uppsats innehåller beskrivningar, pseudokod samt implementationer för alla valsystem. Ett program för att generera valdata utvecklades för att testa de olika systemen. Valsystemen testades i både enmans- och flermansval. Inga nya upptäckter presenteras, men varje systems beteende undersökts experimentellt. Flera slutsatser dras från de olika systemens beteende, inklusive hur proportionella resultat systemen producerade.
\end{abstract}
...
\codeblock{code/generator/ideologyConfig.ts}{lst:ideology configuration}{Configuration object for creating ideologies. Allows for changing the variables to produce different types of electorates and election circumstances. Used to test the methods in different circumstances.}
...
\codeblock{code/generator/scenario1.ts}{lst:scenario 1}{Configuration object for scenario 1. It features 8 candidates, 1 seat to be elected, and a relatively even spread of votes.}
\begin{figure}[H]
	\centering
	\begin{tikzpicture}
		\begin{axis}[
...
\codeblock{code/generator/scenario2.ts}{lst:scenario 2}{Configuration object for scenario 2. It features 9 candidates, 3 seats to be elected, and a ``normal'' spread of votes.}
\begin{figure}[H]
	\centering
	\begin{tikzpicture}
		\begin{axis}[
...
\codeblock{code/generator/scenario3.ts}{lst:scenario 3}{Configuration object for scenario 3. It features 9 candidates, 3 seats to be elected, and a steep spread of votes.}
\begin{figure}[H]
	\centering
	\begin{tikzpicture}
		\begin{axis}[
...
\end{figure}
\codeblock{code/stv/candidateNode.ts}{lst:stv candidate node}{The CandidateNode class. Includes a list of child nodes and methods to manipulate the node.}
...
\codeblock{code/stv/distribute.ts}{lst:stv distribute function}{The distribute function. Main method used to manipulate the state of the election.}
...
\codeblock{code/schulze/algorithm.ts}{lst:schulze algorithm}{Main algorithm for calculating the outcome in the Schulze method. Variant of the Floyd-Warshall algorithm.}
\section{Results}
